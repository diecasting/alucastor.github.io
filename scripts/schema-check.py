...
def extract_jsonld(html_content):
    ...
    # NOTE: `hugo --minify` strips quotes around attribute values, producing
    # `<script type=application/ld+json>`. The quotes must therefore be
    # optional, otherwise this check silently passes on minified output.
    pattern = (
        r'<script[^>]*\btype=["\']?application/ld\+json["\']?[^>]*>'
        r"(.*?)</script>"
    )
